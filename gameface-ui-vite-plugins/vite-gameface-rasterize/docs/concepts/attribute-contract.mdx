...
- `mix-blend-mode` and `backdrop-filter` — they do not composite source-over, so no texture can replace them. The build refuses them with [RZ001](./diagnostics.mdx#rz001).
...
When one survives the bake, the build reports [RZ025](./diagnostics.mdx#rz025) because it is a per-frame cost. A masked panel
...
share one texture. See [preparing your UI](./authoring.mdx) for the rules that keep that matching stable.
