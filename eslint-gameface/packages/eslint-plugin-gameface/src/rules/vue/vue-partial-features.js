...
import { vueElementTagName, vueStartTagAttributeNamesLower } from "../../utils/vue-element.js";
...
export default {
  meta: {
    ...
    docs: {
      description:
        "For partial Vue template elements, warn when markup uses an attribute tied to a false entry in evidence.checks.",
    ...
    return defineVueTemplateVisitor(context, {
      ...
      VElement(node) {
        ...
        if (mode === "attribute-checks" && index.htmlTagsPartial.has(tag)) {
          ...
          const attrsPresent = vueStartTagAttributeNamesLower(node.startTag);
