...
```bash
npm install --save-dev eslint @eslint/css @html-eslint/eslint-plugin eslint-plugin-gameface
```

Requires **ESLint >= 9.15** (flat config). `@eslint/css` may require a recent Node.js LTS; see that package’s `engines` field.
